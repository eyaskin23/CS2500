...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname hw9-final) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
...
; Interpretation: a guessed letter and its associated status
...
(define (value-in-list?\abs x lox p?)
  (ormap (λ (x1) (p? x x1)) lox))
...
(define (read-dictionary text-file)
  (if (file-exists? text-file) (map string-upcase (read-lines text-file))
      '()))   
...
(define (score s1 s2)
  (local [; helper : 1String 1String -> Pair
          ; Creates a pair of 1strings so that they are able to see if they exist in the string
          (define (helper 1-s1 1-s2)
            (make-pair 1-s1
                       (cond
                         [(string=? 1-s1 1-s2) LS-RIGHT]
                         [(string-in-list? 1-s1 (explode s2)) LS-MISPLACED]
                         (else LS-WRONG))))]
    (mymap2 (explode s1) (explode s2)
            helper))) 
...
(define (partial-guess->full-guess string n)
  (string-append string
                 (implode 
                  (build-list (- n (length (explode string))) (λ (x) " ")))))
...
(define (key-wordle ws ke los num)
  (cond [(key=? ke "\b") (backspace-handler ws)] 
        [(key=? ke "\r") (return-handler ws los)]
        [else (other-handler ws ke num)]))  

; backspace-handler: WS -> WS
; processes a backspace key on the WS

(check-expect
 (backspace-handler WS-START)
 WS-START)

(check-expect
 (backspace-handler WS-TYPE1)
 WS-START)
...
(check-expect
 (backspace-handler WS-TYPE3)
 WS-TYPE2)

(define (backspace-handler ws)
  (if (= (string-length (wordle-current ws)) 0)
      ws
      (make-wordle (wordle-past ws) (substring (wordle-current ws) 0
                                               (- (string-length (wordle-current ws)) 1)))))

; return-handler: WS [List-of String] -> WS
; processes a return key on the WS

(check-expect (return-handler WS-START LITTLE-WORDS)
              WS-START)

(check-expect (return-handler WS-TYPE-BAD LITTLE-WORDS)
              WS-TYPE-BAD)

(check-expect (return-handler WS-TYPE1 LITTLE-WORDS)
              WS-TYPE1)

(check-expect (return-handler WS-TYPE1 NYT-WORDS) 
              WS-TYPE1)

(define (return-handler ws los)
  (if (ormap (λ (str) (string=? str (wordle-current ws))) los)
...
; other-handler: WS KeyEvent Nat -> WS
; processes any other key on the WS

(check-expect (other-handler WS-TYPE-BAD "a" 3) WS-TYPE-BAD)
(check-expect (other-handler WS-WORD1 "" 3) WS-WORD1)
(check-expect (other-handler WS-WORD2 "" 3) WS-WORD2)

(check-expect (other-handler WS-START "C" 3)
              WS-TYPE1)

(check-expect (other-handler WS-START "c" 3)
              WS-TYPE1)

(check-expect (other-handler WS-START "1" 3)
              WS-START)

(check-expect (other-handler WS-TYPE1 "A" 3)
              WS-TYPE2)

(define (other-handler ws ke num)
  (if (and (> num (string-length (wordle-current ws))) (ormap (λ (str)
                                                                (string-ci=? str ke)) ALPHABET))
...
; produces the game end screen given the total number of guesses and answer

(check-expect (draw-end-wordle (make-wordle (list "CAT" "DOG" "RED") "") "YES" 3)
              (text "LOSER!" 20 "red"))
(check-expect (draw-end-wordle (make-wordle (list "ABC" "ABC" "ABC" "ABC") "") "CAT" 4)
              (text "LOSER!" 20 "red"))
(check-expect (draw-end-wordle (make-wordle (list "RED") "") "RED" 2)
              (text (string-append "Congrats! You won in 1 tries.") 20 "red"))
(check-expect (draw-end-wordle (make-wordle (list "ABC" "ABC" "ABC" "DEF") "") "DEF" 4)
              (text (string-append "Congrats! You won in 4 tries.") 20 "red"))

(define (draw-end-wordle ws s nat)
  (cond
    [(string-in-list? s (wordle-past ws))
     (text (string-append "Congrats! You won in " (number->string
                                                   (length (wordle-past ws))) " tries.")
           20 "red")]
    [(>= (length (wordle-past ws)) nat)
     (text "LOSER!" 20 "red")]))
...
; draw-past-guesses : [List-of String] String -> Image
; Draws the past guesses as a grid in the wordle. 

(check-expect (draw-past-guesses
               (wordle-past WS-START)
               "BAD")
              '())

(check-expect (draw-past-guesses
               (wordle-past WS-WORD1)
               "CAT")
              (list
               (row->image
                (list
                 (make-pair (make-pair "C" LS-RIGHT) lsp->image)
                 (make-pair (make-pair "A" LS-RIGHT) lsp->image)
                 (make-pair (make-pair "T" LS-RIGHT) lsp->image)))))

(check-expect (draw-past-guesses
               (wordle-past WS-WORD2)
               "DAB")
              (append
               (list
                (row->image
...
(check-expect (draw-past-guesses
               (wordle-past WS-TYPE3)
               "BAD")
              '())

                  
(define (draw-past-guesses los str)
  (local [; list-of-list-of-lsp
          (define SCORED-GUESSES
            (map (λ (_) (score _ str)) los))
          (define SCORED-GUESSES1
            (map (λ (sg) (row->image
                          (map (λ (lsp) 
                                 (make-pair lsp lsp->image))
                               sg))) SCORED-GUESSES))] SCORED-GUESSES1)) 

  
; draw-remaining-guesses: Nat Nat -> [List-of Image]
; draws the remaining guesses in the game given the number of
; guesses remaining and the length of each guess

(check-expect (draw-remaining-guesses 3 4)
              (list (partial-guess->image "" 4)
                    (partial-guess->image "" 4)
                    (partial-guess->image "" 4)))

(check-expect (draw-remaining-guesses 2 5)
              (list (partial-guess->image "" 5) (partial-guess->image "" 5)))

(define (draw-remaining-guesses guesses length)
  (build-list
   guesses (λ (x) (partial-guess->image "" length)))) 
...
(check-expect (draw-wordle WS-TYPE1 "BAD" 3)
              (overlay/align "left" "middle"
                             (stack/v
                              (append (draw-past-guesses '() "BAD")
                                      (list (partial-guess->image "C" 3))
                                      (draw-remaining-guesses 3 3)))
                             BG))

(check-expect (draw-wordle WS-WORD2 "BAD" 4)
              (overlay/align "left" "middle"
                             (stack/v              
                              (append (draw-past-guesses (list "CAT" "DAB") "BAD")
                                      (list (partial-guess->image "" 3))
                                      (draw-remaining-guesses 2 3)))
                             BG))
 
(check-expect (draw-wordle WS-WORD1 "DAB" 5)
              (overlay/align "left"
                             "middle"
                             (stack/v              
                              (append (draw-past-guesses (list "CAT") "DAB")
                                      (list (partial-guess->image "" 3))
                                      (draw-remaining-guesses 4 3)))
                             BG))

(define (draw-wordle ws ans guesses)
  (local [(define PAST (draw-past-guesses (wordle-past ws) ans))
          (define CURRENT (partial-guess->image (wordle-current ws) (string-length ans)))  
          (define REMAINING-GUESSES (draw-remaining-guesses (- guesses (length (wordle-past ws)))
                                                            (string-length ans)))]
    (overlay/align "left" "middle"
                   (stack/v
                    (append
    ...
    (wordle-past
     (big-bang WS-START
       ...
       [stop-when (λ (ws) (end-wordle? ws UP-CORRECT num-guesses))
                  (λ (ws) (draw-end-wordle ws UP-CORRECT (length (wordle-past ws))))])))) 

; file-contents : List-of Number -> String
; 
(define (file-contents lon)
  (foldr
   (λ (s old-s) (string-append s "\n" old-s))            
   ""
   (map number->string lon)))   
 
(define TEXT-SIZE 64)
(define TEXT-COLOR "red")
(define BG (rectangle 1000 1000 "outline" "white")) 
