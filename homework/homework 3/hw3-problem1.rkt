...
(define SC-B2 (make-sc "B" 2))
(define SC-C2 (make-sc "C" 2))
...
; constructor: (make-sc str count sc)

; predicate : sc?: Any -> Boolean
...
(define (increment-if-matches str sc)
 (if (string=? (sc-str sc) str)
        (make-sc str (+ 1 (sc-count sc)))
         sc)) 
