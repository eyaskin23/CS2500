...
(define (in-graph? id g)
  (ormap (λ (x) (= id x)) (graph-v g)))
...
(define (change-color c1 c2 g)
  (local [; same-color? : Graph C1 C2 -> Graph
          ; checks if the supplied graph already has the correct color
          (define (same-color? g c1 c2)
            (λ (n) (if (string=? ((graph-c g) n) c1) c2 ((graph-c g) n))))]
    (make-graph (graph-v g) (same-color? g c1 c2) (graph-e g))))
...
(define (has-a-dot? g)
  (local [; not-a-neighbor? : Nat -> Boolean
          ; determines if no other ids connect to this id
          (define (not-a-neighbor? n)
            (not (ormap (λ (d) ((graph-e g) n d)) (graph-v g))))

          ; no-neighbors?: Nat -> Boolean
          ; determines if this id does not connect to any other ids
          (define (no-neighbors? n)
            (not (ormap (λ (s) ((graph-e g) s n)) (graph-v g))))

          ; lonely-node? : Nat -> Boolean
          ; is this node lonely?
          (define (lonely-node? n)
            (and (not-a-neighbor? n)
                 (no-neighbors? n)))]
    (ormap lonely-node? (graph-v g))))
...
(define (can-reach-in-time? start end steps g)
  (cond
    [(zero? steps) (= start end)]
    [(positive? steps)
     (or (= start end)
         (ormap (λ (d) (if ((graph-e g) start d)
                           (can-reach-in-time? d end (sub1 steps) g)
                           #f)) (graph-v g)))]))
