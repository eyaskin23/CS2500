...
; get-color : LetterStatusPair -> Color
; chooses a color depending on the status of the LSP

(check-expect (get-color LSP-A-R) "darkgreen")
(check-expect (get-color LSP-B-W) "dimgray")
(check-expect (get-color LSP-C-M) "goldenrod")

(define (get-color lsp)
  (cond [(string=? (lsp-status lsp) LS-RIGHT) "darkgreen"]
        [(string=? (lsp-status lsp) LS-WRONG) "dimgray"]
        [(string=? (lsp-status lsp) LS-MISPLACED) "goldenrod"]))
...
(define (lsp->image lsp)
  (overlay
   (text (lsp-letter lsp) (/ LT-SIZE 2) BG-COLOR)
   (square LT-SIZE "solid" (get-color lsp))))
