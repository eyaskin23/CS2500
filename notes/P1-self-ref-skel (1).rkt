...
#reader(lib "htdp-beginner-reader.ss" "lang")((modname |P1-self-ref-skel (1)|) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
; First, finish the move-moon and draw-moon functions

; Then recall our final thoughts: how would we handle something like modelling Jupiter's 80+ moon?
; Let's start with the airline flights, except this time, instead of the pilots, we want to
; support a large set of passengers
...
(define (add-moon s ke)
  ...
  (...
   (cond
     [(boolean? s) (make-system (make-moon NEW-MOON-X NEW-MOON-VX)) s]
     [(system? s) (make-system (system-moon s))
                       (add-moon (system-system s))])))
;<DO NOW>
  )
