...
#reader(lib "htdp-beginner-reader.ss" "lang")((modname P2-true-false) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
...
; represents a single question and correct answer
; - prompt is the question string;
; - answer is the correct T/F answer
;<DO NOW>
...
; A TFTest is a (make-tft (make-tfq TFQuestion TFQuestion TFQuestion TFQuestion TFQuestion)
; representing five true-false questions on a test
; - q1 - q5: the first questions
...
(define (score tfq default-ans)
  (if (boolean =? (tfq answer tfq) default-ans)
      1
      0))
...
(>=
 (+
  (score (tft-q1 tft) answer)
  (score (tft-q2 tft) answer)
  (score (tft-q3 tft) answer)
  (score (tft-q4 tft) answer)
  (score (tft-q5 tft) answer))
 3))
