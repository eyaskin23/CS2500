...
(define (viable-clubs los lost nat)
  (local [; students-and-interests
          ; combines a student with their interests
          (define (students-and-interests s)
            (map student-interests lost))
          (define (produce-list lost)
            (>= (length (students-and-interests lost))
                nat))]
    (filter produce-list los)))
